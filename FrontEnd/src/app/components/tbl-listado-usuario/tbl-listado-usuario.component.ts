...
export class TblListadoUsuarioComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  columnas: ColumnaFiltro[] = [
    ...
    {key:'fathersLastName',canFilter: true, canShow: true, filter:true, nombre:'Ap. Paterno', selected: false, export: true, show: true, type: 'string'},
    {key:'mothersLastName',canFilter: true, canShow: true, filter:true, nombre:'Ap. Materno', selected: false, export: true, show: true, type: 'int'},
    {key:'address',canFilter: true, canShow: true, filter:true, nombre:'Dirección', selected: false, export: true, show: true, type: 'float'},
    {key:'ubigeoCode',canFilter: true, canShow: true, filter:true, nombre:'Ubigeo', selected: false, export: true, show: true, type: 'string'},
    {key:'phone',canFilter: true, canShow: true, filter:true, nombre:'Teléfono', selected: false, export: true, show: true, type: 'DateTime'},
    {key:'email',canFilter: true, canShow: true, filter:true, nombre:'Correo', selected: false, export: true, show: true, type: 'string'},
    {key:'active',canFilter: true, canShow: true, filter:true, nombre:'Activo', selected: false, export: true, show: true, type: 'string'},
  ];
  ...
  evtOnUpdate(): void{
    ...
    this.router.navigate([`/usuario/editar/${data.documentNumber}`])
  }

  evtOnDelete(): void{
    const data = this.selection.selected[0]!;
    const sub = this.api.delete(data.documentNumber).subscribe((res: boolean) => {
